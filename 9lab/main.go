...
func (s *server) selectClub(id int) ClubInfo {
	rows := s.db.QueryRow("select club_id, club_name, price from clubs where club_id=?;", id)
...
func (s *server) updateClubByID(w http.ResponseWriter, r *http.Request) {
	...
	updateClub(Club_name, Price, idInt, s)
...
func (s *server) form1Handle(w http.ResponseWriter, r *http.Request) {
	err := r.ParseForm()
	if err != nil {
		log.Fatal(err)
	}
	...
	outputCHTML(w, "./static/formCmClub.html", club)
}

func createClub(club_name string, price string, s *server) int {
	res, err := s.db.Exec("INSERT INTO clubs (Club_name, Price) VALUES ($1, $2 )", club_name, price)
	if err != nil {
		log.Fatal(err)
	}
...
func updateClub(club_name string, price string, id int, s *server) int {
	res, err := s.db.Exec("update clubs set Club_name=?, price=?  where club_id=?", club_name, price, id)
	if err != nil {
		log.Fatal(err)
	}
...
func deleteClub(id int, s *server) {
	_, err := s.db.Exec("delete from clubs where club_id=?", id)
	if err != nil {
		log.Fatal(err)
	}
}
...
func outputCHTML(w http.ResponseWriter, filename string, club ClubInfo) {
	t, err := template.ParseFiles(filename)
	if err != nil {
		log.Fatal(err)
	}
...
func main() {
	...
	http.HandleFunc("/form1", s.form1Handle)
	http.HandleFunc("/users", s.allUsersHandle)
	http.HandleFunc("/clubs", s.allClubsHandle)
	http.HandleFunc("/change", s.allUserChangeHandle)
	http.HandleFunc("/changeClub", s.allClubChangeHandle)
	http.HandleFunc("/update", s.updateUserForm)
	http.HandleFunc("/updateClub", s.updateClubForm)
	http.HandleFunc("/delete", s.deleteUser)
	http.HandleFunc("/deleteClub", s.deleteClub)
	http.HandleFunc("/updateUserByID", s.updateUserByID)
	http.HandleFunc("/updateClubByID", s.updateClubByID)
